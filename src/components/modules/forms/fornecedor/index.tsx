...
import { alterar, cadastrar } from '../../../../services/fornecedorService'
...
import { FormCadastrarFornecedorStyled } from './styles'
import { validation } from './validation'

const FormCadastrarFornecedor: React.FC = () => {
  const { closeModal, getData, getAction } = useModal<Fornecedor>(
  ...
  const { user } = useAuth()
  const initialValues: Fornecedor = dataModal || {
  ...
  const {
    ...
    onSubmit: async (values: Fornecedor) => {
      setSubmitting(true)
      ...
      <ModalFooter className="mt-4 py-0 pt-3">
        {!readOnly && (
          <Button
            ...
            onSucess={() => {
              handleReset(null)
              closeModal(ModalEnum.createFornecedor, fornecedor)
        ...
        <Button
          ...
          onClick={() => {
            handleReset(null)
            closeModal(ModalEnum.createFornecedor)
          }}>
...
export default FormCadastrarFornecedor
