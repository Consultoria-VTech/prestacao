...
const ContextMenuContratoResponsavel = <T extends object = any>({
  onItemClick,
  ...
  const { show } = useContextMenu({
    id: ContextMenuEnum.menuContratoResponsavel,
  })
  ...
  const handleItemClick = useCallback(
    async ({ event, props }) => {
      const contrato: ContratoResponsavel = props.data.original
          ...
          if (result === true) {
            await deletar(props.data.original)
              .then(() => {
                alertDeleteSuccess()
                if (onItemClick)
                  onItemClick<ContratoResponsavel>(action, contrato)
              })
  ...
  const contextMenuItems = useMemo<ContextMenuItem<ContratoResponsavel, any>[]>(
    () => [
      {
  ...
  return {
    idContextMenu: ContextMenuEnum.menuContratoResponsavel,
    items: contextMenuItems,
    displayMenu,
  }
}

export default ContextMenuContratoResponsavel
