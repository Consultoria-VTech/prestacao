...
import { deletar } from '../../../../services/biService'
import {
...
const ContextMenuBi = <T extends object = any>({
  onItemClick,
}: ContextMenuCustomProps<T>): ContextMenuData<T> => {
  const { openModal } = useModal<Bi>(ModalEnum.createBi)

  const { show } = useContextMenu({
    id: ContextMenuEnum.menuBi,
  })

  const displayMenu = ({ e, data }: DisplayMenuProps<Row<Bi>>) => {
    show(e, { props: { id: Number(data.index), data } })
  }

  const handleItemClick = useCallback(
    async ({ event, props }) => {
      const bi: Bi = props.data.original

      const action = event.currentTarget.id as ActionEnum
      switch (action) {
        case ActionEnum.update:
          openModal(ModalEnum.createBi, bi, {
            action: 'update',
          })
          break
        case ActionEnum.delete:
          ...
          if (result === true) {
            await deletar(props.data.original)
              .then(() => {
                alertDeleteSuccess()
                if (onItemClick) onItemClick<Bi>(action, bi)
              })
  ...
  const contextMenuItems = useMemo<ContextMenuItem<Bi, any>[]>(
    () => [
      {
  ...
  return {
    idContextMenu: ContextMenuEnum.menuBi,
    items: contextMenuItems,
    displayMenu,
  }
}

export default ContextMenuBi