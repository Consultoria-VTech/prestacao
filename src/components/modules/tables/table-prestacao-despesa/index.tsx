...
export const TablePrestacaoDespesa = ({
  ...
  const [aprovadoData, setAprovadoData] = useState(false)
  const [reprovadoData, setReprovadoData] = useState(false)
  ...
  function Aprovadoounao(){
      dataFetch.data.filter((item, indice) => {
        if (item.prestacaoContas?.id === id){
          if(item.status != PrestacaoDespesaStatus.Aprovado){
            console.log("aprovado")
            return setAprovadoData(true)
          }
        }
      })
    }
  ...
  function Reprovadoounao(){
      console.log("ee", reprovadoData)
      dataFetch.data.filter((item, indice) => {
        if (item.prestacaoContas?.id === id){
          if(item.status != PrestacaoDespesaStatus.Reprovado){
            console.log("reprovado")
            return setReprovadoData(true)
          }
        }
      })
    }
        ...
        <div className="text-end align-self-end col-sm-6">
          ...
          {situacao?.length > 0 &&
            situacao[0]?.prestacaoContas?.situacao ===
              ...
              user.usuarioTipo.idUsuarioTipo === 1 &&
              (
              <>
                {/* <Button
                  type="button"
