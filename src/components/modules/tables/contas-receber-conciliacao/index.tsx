...
import { RiFilter2Fill, RiArrowDownSLine } from 'react-icons/ri'
...
const TableContasReceberConciliacao: React.FC<InitialData<ContasReceberPagination>> = ({data: initialData}) => {
  const { data: dataBanksAccount } = useFetch(`/api/contasbancarias/consultar`, {
      ...
      <TableHeader>
          {/* <DivSelectAccount>
            <div className="styled-select">
              <select>
                <option selected disabled >Selecione Banco e Conta</option>
                {dataBanksAccount ? dataBanksAccount.map(item => {                  
                  return (
                    <>
                    <option>{`${item.banco.name} | AG: ${item.agencia}-${item.agenciaDv} | C: ${item.conta}-${item.contaDv}`}</option>
                    </>
                  )
                }) : false}
              </select>
...
export default TableContasReceberConciliacao
